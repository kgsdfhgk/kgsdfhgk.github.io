...
만약 $a[\frac{n}{2}] < a[\frac{n}{2}-1]$ 이라면 $\frac{n}{2}$ 의 왼쪽인 1 ~ $\frac{n}{2}-1$만 확인하고 만약 $a[\frac{n]{2}] \lt a[\frac{n}{2}+1]$ 이라면 오른쪽만 확인하고 둘 다 아니라면 $a[\frac{n}{2}]$가 극댓값인 것을 확인할 수 있는 알고리즘이다.
...
3. $(i, j-1)$값이 더 크면 왼쪽 열을 선택, $(i, j+1)$값이 더 크면 오른쪽 열을 선택
